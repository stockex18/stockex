...
async def add_funds(
    ...
    # NO ledger line here, deliberately. Coins are the platform's internal
    # balance; a ledger line is real money that arrived by cheque or UPI. They
    # happen at different times, in different amounts, and either can happen
    # without the other — so posting one from the other made both wrong. The
    # super admin records the money separately, on the Ledgers page.
...
async def deduct_funds(
    ...
    await wallet_service.adjust(child.id, -amt, transaction_type=TransactionType.ADMIN_WITHDRAW,
                                     ...
                                     payment_mode=payment_mode)
    # No ledger line — see `add_funds`. The money side is recorded by hand.
    await wallet_service.adjust(actor.id, amt, transaction_type=TransactionType.ADMIN_TRANSFER,
